...
def safe_open(pth, mode):
    ...
    try:
        ...
        # Check if the file is opened in writing mode and has content
        if 'w' in mode and file.readable() and file.read(1):
            print(f"File '{pth}' already has content. Refusing to open in 'w' mode.")
            file.close()
            return None

        # Reset file position to the beginning if opened in read mode
        if 'r' in mode:
            file.seek(0)

        return file
    except Exception as e:
        print(f"Error opening file '{pth}': {str(e)}")
        return None
